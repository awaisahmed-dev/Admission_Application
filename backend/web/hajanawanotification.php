...
$conn->set_charset("utf8"); // Added to tell server this os utf8
$url = 'https://hajanaone.com/api/waAPI.php';
        ...
        if (preg_match( "/API key Missing/", $result )){
		...
		function SendNOtificationFromHajana($schoolid = 22){
            ...
            //$notificationCount = 0;
        global $conn;
        $sql = "SELECT * FROM notification where school_id='".$schoolid."' AND (medium ='whatsapp' or medium = 'null') AND (STATUS = 555 or STATUS = 555) and created_at > ".strtotime(date('Y-m-d', strtotime( '-2 day') ) )." ORDER BY id DESC LIMIT 2";
...
//            $noti->api_response = $requests[$k]['content'];
            ...
            $notificationCount++;
            if (preg_match( "/success/", $requests[$k]['content'] )){
...
//        $oper   =   1;                                      //  If Number is Ported to telenor
       ...
       return $url = 'https://hajanaone.com/api/waAPI.php?apikey='.$api.'&waid='.$waid.'&phone='.$number."&type=$type".'&message='.rawurlencode($text); //.'&operator='.$oper; //rawurlencode removed for urdu text
